...
def create_transforms(config=None, split='train', is_eval=False):
    if config is None or config.transforms.type == 'imagenet256x256':
    ...
    elif 'ffhq' in config.transforms.type or 'cc3m' in config.transforms.type or 'coco' in config.transforms.type or 'pretrain' in config.transforms.type:
