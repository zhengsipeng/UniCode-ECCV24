...
from src.rqvae.optimizer.loss import torch_compute_entropy
...
class SummaryStage1WithGAN:
    def __init__(self, ent_codes_w_pad, ent_codes_wo_pad, codebooks, **kwargs):
    ...
    def print_line(self):
        line = ""
        line += f"code usage: %d "%(self.codebooks[0]>0).sum().item()
    ...
    def print_line_short(self):
        line = ""
        
        for name, value in self.metrics.items():
            if name=="lr":
                continue
            #if name not in ['loss_total', 'loss_recon']:
            #    continue
            line += f"{name}: {value.item():.4f}, "
        return line
...
class AccmStage1WithGAN:
    def __init__(self, metric_names, n_codebook=1, codebook_size=512, code_hier=1, use_padding_idx=False, device='cpu'):
        ...
        self.metrics_cur = {n: torch.zeros(1, device=self.device) for n in metric_names}
        self.codebooks = [torch.zeros(self.n_codebook, self.max_codebook_size, device=self.device)
                          for _ in range(self.code_hier)]
        self.counter = 0
        self.accm_steps = 0
    ...
    def update(self,
        ...
        if distenv is not None:
            world_size = distenv if isinstance(distenv, int) else distenv.world_size

        if sync:
            codes = [dist_utils.all_gather_cat(world_size, code) for code in codes]
            for name, value in metrics_to_add.items():
                gathered_value = dist_utils.all_gather_cat(world_size, value.unsqueeze(0))
        ...
        for name, value in metrics_to_add.items():
            ...
            self.metrics_cur[name] = value
        
        for i in range(self.code_hier):
            assign_code(self.codebooks[i], codes[i].detach())

        self.counter += count if not sync else count * world_size
    ...
    def get_summary(self, n_samples=None):
        ...
        metrics_avg['llm_cos_sim'] = self.metrics_cur['llm_cos_sim']
        metrics_avg['llm_idx_acc'] = self.metrics_cur['llm_idx_acc']
        metrics_avg['llm_same'] = self.metrics_cur['llm_same']
        if self.use_padding_idx:
            ent_codes_w_pad = [torch_compute_entropy(self.codebooks[i]) for i in range(self.code_hier)]
            ent_codes_wo_pad = [torch_compute_entropy(self.codebooks[i][:, :-1]) for i in range(self.code_hier)]
        else:
