...
class VQLPIPSWithDiscriminator(nn.Module):
    ...
    def calculate_adaptive_weight(self, nll_loss, g_loss, last_layer=None, llm_ratio=0):
        ...
        d_weight = torch.clamp(d_weight, 0.0, 5e2).detach()
        d_weight = d_weight * self.discriminator_weight
        return d_weight

    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
                global_step, llm_ratio=0, last_layer=None, cond=None, split="train"):
        ...
        if optimizer_idx == 0:
            ...
            try:
                d_weight = self.calculate_adaptive_weight(nll_loss, g_loss, last_layer=last_layer, llm_ratio=llm_ratio)
            except RuntimeError:
                assert not self.training
                d_weight = torch.tensor(0.0)
           
            g_loss = g_loss * (1-0.9*llm_ratio)
