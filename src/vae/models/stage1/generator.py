...
from .modules.quantizer import VectorQuantizer, EMAVectorQuantizer, RQ_EMAVectorQuantizer
from src.vae.models.stage2.layers import DecoderBlock
...
class VQGAN2Generator(torch.nn.Module):
    ...
    def encode(self, x: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        quant_t, diff_t, diff_llm_t, code_t = self.quantize_t(h_t)
        ...
        if self._shared_codebook:
            quant_b, diff_b, diff_b_llm, code_b = self.quantize_t(h_b)
        else:
            quant_b, diff_b, diff_b_llm, code_b = self.quantize_b(h_b)
        code = (code_t, code_b)
        return quant_t, quant_b, diff_t, diff_b, code, [diff_llm_t, diff_b_llm]
...
class RQGenerator(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.num_quant_layer = hparams_aux.num_quant_layer
  
        self.use_l2_norm = hparams_aux.use_l2_norm
        self.learnable = hparams_aux.learnable
        self.llmcodebook = hparams_aux.llmcodebook
        self.norm_type = hparams_aux.norm_type
        self.fix_len_l2norm = hparams_aux.fix_len_l2norm
        self.tail_epoch = hparams_aux.tail_epoch
        self.restart_unused_codes = hparams_aux.restart_unused_codes
        
        if ema_update:
            self.quantize = RQ_EMAVectorQuantizer(dim=embed_dim, use_l2_norm=self.use_l2_norm, llmcodebook=self.llmcodebook, 
                                                  learnable=self.learnable, n_embed=n_embed, beta=0.25, norm_type=self.norm_type,
                                                  fix_len_l2norm=self.fix_len_l2norm, tail_epoch=self.tail_epoch, restart_unused_codes=self.restart_unused_codes)
        else:
            self.quantize = VectorQuantizer(dim=embed_dim, use_l2_norm=self.use_l2_norm, llmcodebook=self.llmcodebook, learnable=self.learnable, n_embed=n_embed, beta=0.25)
    ...
    def encode(self, x: torch.FloatTensor, global_step=0) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        h = self.quant_conv(self.encoder(x))
        self.quantize.pre_len_l2norm = torch.norm(h, p=2, dim=1).mean()
    
        if self.quantize.pre_len_l2norm>=self.fix_len_l2norm:
            h = h/self.quantize.pre_len_l2norm * self.fix_len_l2norm

        self.quantize.post_len_l2norm = torch.norm(h, p=2, dim=1).mean() / self.fix_len_l2norm
        residual_feature = h
        quant_list = [] # [bs,8,8,256]x4
        code_list = []
        diff = torch.zeros_like(self.quantize.post_len_l2norm)
        
        for _ in range(self.num_quant_layer):
            quant, _diff, _code = self.quantize(residual_feature, global_step=global_step)
            diff.add_(_diff)
            residual_feature.sub_(quant)  # -=quant
      
            quant_list.append(quant.clone())
            code_list.append(_code.unsqueeze(-1))
        
        code = (torch.cat(code_list, dim=-1), h)
        diff /= self.num_quant_layer
   
        return quant_list, diff, code
    
    def decode(self, quant_list: torch.FloatTensor):
        
        quant = torch.stack(quant_list).sum(dim=0)
        quant = self.post_quant_conv(quant)
        #quant = torch.cat([quant_t, quant_b], dim=1)  # b,2*dim,16,16
    ...
    def forward(self, x, global_step=None):
        ...
        quant_list, diff, code = self.encode(x, global_step)
        assert quant_list[0].shape[-1] == self.latent_dim, f'latent dim should be [C, {self.latent_dim}, {self.latent_dim}]'

        diff_cos = 1-torch.cosine_similarity(self.quantize.llmcodebook, self.quantize.embedding, dim=-1).mean()
        diff = (diff, torch.abs(code[1]).mean(), diff_cos)
...
class SimRQGAN2Generator(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if ema_update:
            self.quantize_t = EMAVectorQuantizer(dim=embed_dim_top, use_l2_norm=self.use_l2_norm, llmcodebook=self.llmcodebook, 
                                                 learnable=self.learnable, n_embed=n_embed, beta=0.25, norm_type=self.norm_type,
                                                 fix_len_l2norm=self.fix_len_l2norm, tail_epoch=self.tail_epoch, restart_unused_codes=self.restart_unused_codes)
            self.quantize_b = EMAVectorQuantizer(dim=embed_dim_bot, use_l2_norm=self.use_l2_norm, llmcodebook=self.llmcodebook, 
                                                 learnable=self.learnable, n_embed=n_embed, beta=0.25, norm_type=self.norm_type,
                                                 fix_len_l2norm=self.fix_len_l2norm, tail_epoch=self.tail_epoch, restart_unused_codes=self.restart_unused_codes)
        else:
        ...
        self.quant_conv_b = torch.nn.Conv2d(hparams.z_channels, int(embed_dim/kernel_size/kernel_size), 1)
        self.post_quant_conv_b = torch.nn.Conv2d(int(embed_dim/kernel_size/kernel_size)*2, hparams.z_channels, 1)

        self.up_conv = torch.nn.Conv2d(int(embed_dim/kernel_size/kernel_size), embed_dim, 1)
        self.down_conv = torch.nn.Conv2d(embed_dim, int(embed_dim/kernel_size/kernel_size), 1)
        ...
        self.embed_dim_output = int(embed_dim/kernel_size/kernel_size)*2
    ...
    def forward(self,
        ...
        if (global_step is not None and global_step >= self._bottom_start and self.training):
            ...
            quant_t, quant_b, diff_t, diff_b, code = self.encode(x, global_step=global_step)  # b,1024,8,8; b,256,16,16
            dec = self.decode(quant_t, quant_b)
        
        diff_cos = 1-torch.cosine_similarity(self.quantize_t.llmcodebook, self.quantize_t.embedding, dim=-1).mean()
       
        diff = (diff_t, diff_b, torch.abs(code[2]).mean(), diff_cos)
        return dec, diff, code
    ...
    def forward_quant(self, x):
        h_b = self.quant_conv_b(self.encoder(x))  # b,1024,16,16
        h_t = self.down_t(h_b)  # b,4096,8,8

        quant_t, diff_t, code_t = self.quantize_t(h_t)

        h_b = h_b - self.upsample_t(quant_t)
        
        h_b = self.up_conv(h_b)  
        if self._shared_codebook:
    ...
    def forward_encode(self, x):
        ...
        if self._shared_codebook:
            ...
            quant_b, diff_b, code_b = self.quantize_b(h_b)
        import pdb;pdb.set_trace()
        quant_b = self.down_conv(quant_b)
        # merge quant_t and quant_b
        quant_t = self.upsample_t(quant_t)
    ...
    def encode(self, x: torch.FloatTensor, global_step=0) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        quant_t, diff_t, code_t = self.quantize_t(h_t, global_step=global_step)
     
        h_b = h_b - self.upsample_t(quant_t)
        
        h_b = self.up_conv(h_b)  
        if self._shared_codebook:
            quant_b, diff_b,  code_b = self.quantize_t(h_b, global_step=global_step)
        else:
            quant_b, diff_b, code_b = self.quantize_b(h_b, global_step=global_step)
        quant_b = self.down_conv(quant_b)
    ...
    def decode(self,
               quant_t: torch.FloatTensor,
               quant_b: torch.FloatTensor) -> torch.FloatTensor:
    ...
    def get_soft_codes(self, xs, temp=1.0, stochastic=False):
        ...
        if self.use_l2_norm:
            h_b = F.normalize(h_b, p=2.0, dim=1, eps=1e-6)
...
class HQVAEGenerator(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_dim_output = embed_dim
