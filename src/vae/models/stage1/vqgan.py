...
class VQGAN(pl.LightningModule):
    ...
    def forward(self, x: torch.FloatTensor, global_step=0) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        xrec, qloss, _ = self(batch[0], global_step=self.global_step)

        if isinstance(qloss, tuple) or isinstance(qloss, list):
            llm_cos_diff = qloss[-1]
            
            if (self._residual_l1_weight > 0.0):
                resid_l1_loss = qloss[-2]
                self.log("train/resid_l1_loss", resid_l1_loss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            
            diff = qloss[0] if len(qloss)==3 else sum(qloss[0:len(qloss)-2])
            qloss = diff 

        llm_ratio = self.generator.quantize.ratio if hasattr(self.generator, "quantize") else self.generator.quantize_t.ratio
        if optimizer_idx == 0:
            # autoencode  
            aeloss, log_dict_ae = self.discriminator(qloss, batch[0], xrec, optimizer_idx, self.global_step, llm_ratio=llm_ratio,
            ...
            self.log("train/diff_cos", llm_cos_diff, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log("train/diff", diff, prog_bar=True, logger=True, on_step=True, on_epoch=True)
       
            pre_len_l2norm = self.generator.quantize.pre_len_l2norm if hasattr(self.generator, "quantize") else  self.generator.quantize_t.pre_len_l2norm
            self.log("train/pre_len_l2norm", pre_len_l2norm, prog_bar=True, logger=True, on_step=True, on_epoch=True)
        ...
        if optimizer_idx == 1:
            # discriminator
            discloss, log_dict_disc = self.discriminator(qloss, batch[0], xrec, optimizer_idx, self.global_step, llm_ratio,
                                                         last_layer=self.get_last_layer(), split="train")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if isinstance(qloss, tuple) or isinstance(qloss, list):
            llm_cos_diff = qloss[-1]
            self.log("valid/diff_cos", llm_cos_diff, prog_bar=True, logger=True, on_step=True, on_epoch=True)

            diff = qloss[0] if len(qloss)==3 else sum(qloss[0:len(qloss)-2])    
            qloss =  diff

        llm_ratio = self.generator.quantize.ratio if hasattr(self.generator, "quantize") else self.generator.quantize_t.ratio
        aeloss, log_dict_ae = self.discriminator(qloss, batch[0], xrec, 0, self.global_step, llm_ratio,
        ...
        self.log("valid/diff", diff, prog_bar=True, logger=True, on_step=True, on_epoch=True)       
        pre_len_l2norm = self.generator.quantize.pre_len_l2norm if hasattr(self.generator, "quantize") else  self.generator.quantize_t.pre_len_l2norm
        self.log("valid/pre_len_l2norm", pre_len_l2norm, prog_bar=True, logger=True, on_step=True, on_epoch=True)  
        #post_len_l2norm = self.generator.quantize.post_len_l2norm if hasattr(self.generator, "quantize") else  self.generator.quantize_t.post_len_l2norm
        #self.log("valid/post_len_l2norm", post_len_l2norm, prog_bar=True, logger=True, on_step=True, on_epoch=True)  
